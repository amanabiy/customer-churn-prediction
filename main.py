...
def prepare_input(credit_score, location, geneder, age, tenure, balance,
...
def explain_prediction(probability, input_dict, surname):
  ...
  prompt = f"""
    ...
    {round(probability * 100, 1)}% probability of churning, based on the
  ...
  - If the customer has less than a 40% risk of cchurning, generate a 3 sentence
  explanation of why they might not be at risk of churning.
  - Your explanation should be based on the customer's information and the summary
  ...
  row_response = client.chat.completions.create(model="llama-3.2-3b-preview",
                                                messages=[{
                                                    "role": "user",
                                                    "content": prompt
                                                }])

  return row_response.choices[0].message.content


def generate_email(probability, input_dict, explanation, surname):
  prompt = f"""
    You are a manager at HS Bank, where you are responsible for eduring customers stay with the bank and are incentivized with various offers.
    ...
    Sign the Email saying Manager at HS Bank
  """

  row_response = client.chat.completions.create(model="llama-3.1-8b-instant",
                                                messages=[{
                                                    "role": "user",
                                                    "content": prompt
                                                }])
